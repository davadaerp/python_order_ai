...
def process_order_parsing(row):
    주문일자 = row['주문일자']    # 주문일자
    업체코드 = row['업체코드']    # 업체코드
    사업자번호 = row['사업자번호'] # 사업자번호
    업체명 = row['업체명']       # 업체명
    지역 = row['지역']          # 지역: 서울/경기/인천
    지역상세 = row['지역상세']    # 시/군/구
    업태 = row['업태']          # 업태: business_category
    업종 = row['업종']          # 업종: business_type
    업종상세 = row['업종상세']    # 업종상세: business_detail 고기,부폐,코드
    주소 = row['주소']          # 주소
    #
    원주문내용 = row['주문내용']  # 원주문내용에 저장
    #
    parsing_result = []
    #
...
# order parsing start
results = process_orders_file()

# url json호출방식 처리
#results = process_orders_json()
